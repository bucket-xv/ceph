...
namespace
{
                                      ...
                                      const std::string *pool_name)
  {
    ...
    if (mon_caps.is_capable(
            ...
            (pool_name == nullptr ? CommandArgs{} /* pool DNE, require unrestricted cap */ : CommandArgs{{"poolname", *pool_name}}),
            false, true, false,
            peer_socket_addr))
    {
      return true;
...
public:
  ...
  utime_t start;
  epoch_t epoch;
  C_UpdateCreatingPGs(OSDMonitor *osdmon, epoch_t e) : osdmon(osdmon), start(ceph_clock_now()), epoch(e) {}
...
bool OSDMonitor::prepare_alive(MonOpRequestRef op)
{
  ...
  if (0)
  { // we probably don't care much about these
                                 ...
                                 ostream *ss)
{
  ...
  if (pg_num == 0)
  {
    auto pg_num_from_mode =
        [pg_num = g_conf().get_val<uint64_t>("osd_pool_default_pg_num")](const string &mode)
    {
      return mode == "on" ? 1 : pg_num;
    };
    pg_num = pg_num_from_mode(
        pg_autoscale_mode.empty() ? g_conf().get_val<string>("osd_pool_default_pg_autoscale_mode") : pg_autoscale_mode);
  }
  if (pgp_num == 0)
    ...
    Formatter *f)
{
  ...
  bool has_secrets = params.count("cephx_secret") || params.count("cephx_lockbox_secret") || params.count("dmcrypt_key");
                                      ...
                                      const cmdmap_t &cmdmap)
{
  ...
  { // avoid commands with non-int id arg
        ...
        for (auto &q : p->second)
        {
          ...
          if (missing)
          {
            ...
            ss << q.second << " connected " << ceph_entity_type_name(type)
               << "(s) look like " << ceph_release_name(ceph_release_from_features(q.first))
           ...
           prefix == "osd rm-noout")
  {
    ...
    for (auto osd : osds)
    {
      ...
      if (do_set)
      {
        ...
        {
          any |= osdmap.is_noup_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOUP) : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOUP);
        }
        if (flags & CEPH_OSD_NODOWN)
        {
          any |= osdmap.is_nodown_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NODOWN) : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NODOWN);
        }
        if (flags & CEPH_OSD_NOIN)
        {
          any |= osdmap.is_noin_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOIN) : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOIN);
        }
        if (flags & CEPH_OSD_NOOUT)
        {
          any |= osdmap.is_noout_by_osd(osd) ? pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOOUT) : pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOOUT);
        }
      }
      else
      {
        if (flags & CEPH_OSD_NOUP)
        {
          any |= osdmap.is_noup_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOUP) : pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOUP);
        }
        if (flags & CEPH_OSD_NODOWN)
        {
          any |= osdmap.is_nodown_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NODOWN) : pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NODOWN);
        }
        if (flags & CEPH_OSD_NOIN)
        {
          any |= osdmap.is_noin_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOIN) : pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOIN);
        }
        if (flags & CEPH_OSD_NOOUT)
        {
          any |= osdmap.is_noout_by_osd(osd) ? pending_inc.pending_osd_state_set(osd, CEPH_OSD_NOOUT) : pending_inc.pending_osd_state_clear(osd, CEPH_OSD_NOOUT);
        }
      }
    }
           ...
           prefix == "osd rm-pg-upmap-primary")
  {
    ...
    case OP_PG_UPMAP_PRIMARY:
    {
      ...
      int found_idx = 0;
      for (int i = 1; i < (int)acting.size(); i++)
      { // skip 0 on purpose
        if (acting[i] == id)
        {
          found_idx = i;
    ...
    int *err, ostream *ss) const
{
  ...
  if (pending_fsmap.pool_in_use(base_pool_id))
  {
    if (base_pool->is_erasure() && !base_pool->allows_ecoverwrites())
    {
      ...
      *ss << "pool '" << base_pool_name << "' does not allow EC overwrites and is in use by CephFS"
                                           " via its tier";
      *err = -EBUSY;
      return false;
    }
...
void OSDMonitor::convert_pool_priorities(void)
{
  ...
  for (const auto &i : osdmap.get_pools())
  {
    ...
    if (prio > 0 && max_prio > OSD_POOL_PRIORITY_MAX)
    { // Likely scenario
