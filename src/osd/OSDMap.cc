    ...
    const std::optional<rb_policy> &rbp) const
{
  ...
  for(auto &[bytes, pg] : bytes_used_by_pg)
  {
    ...
    for(auto osd : up_osds)
    {
      ...
      if(bytes_used_by_osd[osd] < bytes_used_by_osd[best_primary])
      {
        best_primary = osd;
      }
    }
    bytes_used_by_osd[best_primary] += bytes;
    if(best_primary != up_primary){
      ldout(cct, 10) << __func__ << " pg " << pg << " moving from " << up_primary << " to " << best_primary << dendl;
      pending_inc->set_primary(pg, best_primary);
      ++num_changes;
    }
  }
  return num_changes;
    ...
    const std::optional<rb_policy> &rbp) const
{
  ...
  { // clear out any mappings that were made since the score didn't improve
    for (auto [pg, mapped] : prim_pgs_to_check)
    {
      if (mapped)
      {
  ...
  else
  {
    ...
    while (cont)
    {
      ...
      for (int i = 0; i < num_osds; i++)
      {
        ...
        {
          if (!pgs_used.contains(p))
          { // no mark for the primary of this PG yet
...
public:
  ...
  OSDUtilizationDumper(const CrushWrapper *crush, const OSDMap *osdmap_,
                       ...
                       const string &filter) : Parent(crush, osdmap_->get_pool_names()),
                                               osdmap(osdmap_),
                                               pgmap(pgmap_),
                                               tree(tree_),
                                               min_var(-1),
...
public:
  ...
  OSDUtilizationPlainDumper(const CrushWrapper *crush, const OSDMap *osdmap,
                            ...
                            const string &filter) : Parent(crush, osdmap, pgmap, tree, filter) {}
