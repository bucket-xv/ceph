...
struct PGTempMap
{
  ...
  friend bool operator==(const PGTempMap &l, const PGTempMap &r)
  {
    return l.pg_temp.size() == r.pg_temp.size() &&
           l.pg_temp == r.pg_temp;
  }
...
public:
  ...
  std::map<uint64_t, std::set<pg_t>> get_pgs_by_osd(
      ...
      std::map<uint64_t, std::set<pg_t>> *p_acting_primaries_by_osd = nullptr) const; // used in calc_desired_primary_distribution()
